...
public class Genoplot extends JFrame implements ActionListener {
	...
	Genoplot() {
		...
		loadList
				.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_L, menumask));
		...
		scorePanel.registerKeyboardAction(this, "Maybe", KeyStroke
				.getKeyStroke(KeyEvent.VK_M, 0),
				JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT);
			...
			if (command.equals("Go")) {
				if (markerListLoaded()) {
					if (snpList.contains(snpField.getText().trim())) {
						setHistorySNP(true);
						displaySNPindex = snpList.indexOf(snpField.getText()
								.trim());
					}
				}
				...
				if (jfc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
					try {
						...
						db = new DataDirectory(jfc.getSelectedFile()
								.getAbsolutePath());
				...
				if (jfc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
					...
					File outfile = checkOverwriteFile(new File(jfc
				...
				if (jfc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
					...
					db.setExcludeList(new QCFilterData(jfc.getSelectedFile()
							.getAbsolutePath()));
					...
					while (keys.hasMoreElements() && ! pm.isCanceled()) {
						...
						for (String collection : db.getCollections()) {
							...
							if (pp.hasData()) {
								...
								images.add(Image.getInstance(tempFile
										.getAbsolutePath()));
								stats.add("");
							}
						}
	...
	private void openPDFs() throws DocumentException, IOException {
		...
		if (pdfd.maybePlots()) {
			maybePDF = new EvokerPDF(checkOverwriteFile(new File(pdfd
					.getPdfDir()
	...
	private void closePDFs() throws DocumentException {
		...
		if (pdfd.yesPlots() && yesPDF.isFileOpen()) {
			if (yesPlotNum == 0) {
				yesPDF.getDocument()
						.add(new Paragraph("No Yes plots recorded"));
			}
